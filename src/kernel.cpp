...
extern "C"{
	  ...
	  /************************************************************************/
      /* agglomerative partitioning 3D ****************************************/
      /************************************************************************/
      SEXP agglom3D(   SEXP mz, /* must be sorted */
                       SEXP rt1,
                       SEXP rt2,
                       SEXP ppm,
                       SEXP dmz,
            ...
            PROTECT(mz = AS_NUMERIC(mz));
            PROTECT(rt1 = AS_NUMERIC(rt1));
            PROTECT(rt2 = AS_NUMERIC(rt2));
            PROTECT(ppm = AS_INTEGER(ppm));
            PROTECT(dmz = AS_NUMERIC(dmz));
            PROTECT(drt1 = AS_NUMERIC(drt1));
            PROTECT(drt2 = AS_NUMERIC(drt2));
            double *mass;
            mass = NUMERIC_POINTER(mz);
            double *ret1;
            ret1 = NUMERIC_POINTER(rt1);
            double *ret2;
            ret2 = NUMERIC_POINTER(rt2);
            int ppm2 = INTEGER_VALUE(ppm);
            double dmass = NUMERIC_VALUE(dmz);
            double dret1 = NUMERIC_VALUE(drt1);
            double dret2 = NUMERIC_VALUE(drt2);
            int n, m, p, k = 0, found = 0;
            double lowmass, highmass, lowret1, highret1, lowret2, highret2;
            int leng = LENGTH(rt1);
            SEXP outit;
            PROTECT(outit = NEW_INTEGER(leng));
            int *at;
                    ...
                    while(found == 1){
                        ...
                        if(atthese == 1){
                            ...
                            for(m = 0; m < untilthese; m++){
                                ...
                                lowret1 = (*(ret1 + these[m]) - dret1);
                                highret1 = (*(ret1 + these[m]) + dret1);
                                lowret2 = (*(ret2 + these[m]) - dret2);
                                highret2 = (*(ret2 + these[m]) + dret2);
                                if(these[m] > 0){
                                    for(p = (these[m] - 1); p >= 0; p--){ /* towards lower mass */
                                        if((*(mass + p)) <= (lowmass)){
                                            break;
                                            ...
                                            if(*(at + p) == 0){
                                                if((*(ret1 + p) >= lowret1) && (*(ret1 + p) <= highret1)){
													if((*(ret2 + p) >= lowret2) && (*(ret2 + p) <= highret2)){
														those[untilthose] = p;
														untilthose++;
														*(at + p) = k;
														found = 1;
                            ...
                            for(m = 0; m < untilthose; m++){
                                ...
                                lowret1 = (*(ret1 + those[m]) - dret1);
                                highret1 = (*(ret1 + those[m]) + dret1);
                                lowret2 = (*(ret2 + those[m]) - dret2);
                                highret2 = (*(ret2 + those[m]) + dret2);
                                if(those[m] > 0){
                                    for(p = (those[m] - 1); p >= 0; p--){
                                        if((*(mass + p)) <= (lowmass)){
                                            break;
      ...
      /************************************************************************/
		...
		SEXP getEIC3D( 	SEXP mz,
			...
			PROTECT(dmzdens = AS_NUMERIC(dmzdens));
			PROTECT(ppm2 = AS_INTEGER(ppm2));
			PROTECT(drtdens1 = AS_NUMERIC(drtdens1));
			PROTECT(drtdens2 = AS_NUMERIC(drtdens2));
			PROTECT(merged2 = AS_INTEGER(merged2));
			double *ret1, *ret2, *mass, *intensity;
			mass = NUMERIC_POINTER(mz);
			ret1 = NUMERIC_POINTER(RT1);
			ret2 = NUMERIC_POINTER(RT2);
			intensity = NUMERIC_POINTER(intens);
			...
			clustnumb = 1;
			if(ppm3 == 1){delmz = ((dmzdens2**(mass + (*(ordint) - 1))) / 1e6);}else{delmz = dmzdens2;}
			clus[(0 * leng)] = (*(mass + (*(ordint) - 1)) - (2 * delmz)); 	// low mass boundary
			clus[(1 * leng)] = (*(mass + (*(ordint) - 1)) + (2 * delmz));	// high mass boundary
			clus[(2 * leng)] = (*(ret1 + (*(ordint) - 1)) - drtdens_1);  	// low RT1 boundary
			clus[(3 * leng)] = (*(ret1 + (*(ordint) - 1)) + drtdens_1);  	// high RT1 boundary
			clus[(4 * leng)] = 1;                                			// number of measurements
			...
			clus[(11 * leng)] = *(mass + (*(ordint) - 1));           		// lowest centroid mass in cluster
			clus[(12 * leng)] = *(mass + (*(ordint) - 1));           		// highest centroid mass in cluster
			clus[(13 * leng)] = (*(ret2 + (*(ordint) - 1)) - drtdens_2);  	// low RT2 boundary
			clus[(14 * leng)] = (*(ret2 + (*(ordint) - 1)) + drtdens_2);  	// high RT2 boundary			
			...
			for(n = 1; n < leng; n++){
				...
				for(m = 0; m < clustnumb; m++){
					if(*(mass + (*(ordint + n) - 1)) <= clus[(1 * leng) + m]){ // below high mass boundary?
						if(*(mass + (*(ordint + n) - 1)) >= clus[(0 * leng) + m]){ // above low mass boundary?
							if(*(ret1 + (*(ordint + n) - 1)) >= clus[(2 * leng) + m]){ // above low RT1 boundary?
								if(*(ret1 + (*(ordint + n) - 1)) <= clus[(3 *leng) + m]){ // below high RT1 boundary?	
									if(*(ret2 + (*(ordint + n) - 1)) >= clus[(13 * leng) + m]){ // above low RT2 boundary?
										if(*(ret2 + (*(ordint + n) - 1)) <= clus[(14 *leng) + m]){ // below high RT2 boundary?
											at[maxat] = m; // store candidate cluster
											maxat++;
										}								
									}
				...
				if(maxat == 0){
					...
					clus[0 + (clustnumb - 1)] = (*(mass + (*(ordint + n) - 1)) - (2 * delmz));
					clus[(1 * leng) + (clustnumb - 1)] = (*(mass + (*(ordint + n) - 1)) + (2 * delmz));
					clus[(2 * leng) + (clustnumb - 1)] = (*(ret1 + (*(ordint + n) - 1)) - drtdens_1);
					clus[(3 * leng) + (clustnumb - 1)] = (*(ret1 + (*(ordint + n) - 1)) + drtdens_1);
					clus[(4 * leng) + (clustnumb - 1)] = 1;
					clus[(5 * leng) + (clustnumb - 1)] = *(mass + (*(ordint + n) - 1));
					clus[(6 * leng) + (clustnumb - 1)] = clustnumb;
					clus[(7 * leng) + (clustnumb - 1)] = 0;
					...
					clus[(11 * leng) + (clustnumb - 1)] = *(mass + (*(ordint + n) - 1));
					clus[(12 * leng) + (clustnumb - 1)] = *(mass + (*(ordint + n) - 1));
					clus[(13 * leng) + (clustnumb - 1)] = (*(ret2 + (*(ordint + n) - 1)) - drtdens_2);
					clus[(14 * leng) + (clustnumb - 1)] = (*(ret2 + (*(ordint + n) - 1)) + drtdens_2);
					continue; // go to next centroid
				}
				...
				if(k > 0){ 
					for(i = (k - 1);i >= 0; i--){ // backward over RT-order
						if(  
							(*(ret1 + (*(ordret + k) - 1)) == *(ret1 + (*(ordret + i) - 1)) ) &&
							(*(ret2 + (*(ordret + k) - 1)) == *(ret2 + (*(ordret + i) - 1)) ) 						
						){ // same RT?
							if((clus[(9 * leng) + (*(ordret + i) - 1)]) == 0){ // not in cluster yet
								continue;
							}else{
				...
				if(maxat == 1){
					...
					if(clus[(1 * leng) + (at[0])] > (*(mass + (*(ordint + n) - 1)) + (2 * delmz))){clus[(1 * leng) + (at[0])] = (*(mass + (*(ordint + n) - 1)) + (2 * delmz));}
					/* adapt lower & upper RT bounds */
					if(clus[(2 * leng) + (at[0])] > (*(ret1 + (*(ordint + n) - 1)) - drtdens_1)){clus[(2 * leng) + (at[0])] = (*(ret1 + (*(ordint + n) - 1)) - drtdens_1);}
					if(clus[(3 * leng) + (at[0])] < (*(ret1 + (*(ordint + n) - 1)) + drtdens_1)){clus[(3 * leng) + (at[0])] = (*(ret1 + (*(ordint + n) - 1)) + drtdens_1);}
					clus[(4 * leng) + (at[0])] = clus[(4 * leng) + (at[0])] + 1;
					clus[(5 * leng) + (at[0])] = clus[(5 * leng) + (at[0])] + *(mass + (*(ordint + n) - 1));
					if(*(mass + (*(ordint + n) - 1)) < clus[(11 * leng) + (at[0])]){clus[(11 * leng) + (at[0])] = *(mass + (*(ordint + n) - 1));};
					if(*(mass + (*(ordint + n) - 1)) > clus[(12 * leng) + (at[0])]){clus[(12 * leng) + (at[0])] = *(mass + (*(ordint + n) - 1));};
					if(clus[(13 * leng) + (at[0])] > (*(ret2 + (*(ordint + n) - 1)) - drtdens_2)){clus[(13 * leng) + (at[0])] = (*(ret2 + (*(ordint + n) - 1)) - drtdens_2);}
					if(clus[(14 * leng) + (at[0])] < (*(ret2 + (*(ordint + n) - 1)) + drtdens_2)){clus[(14 * leng) + (at[0])] = (*(ret2 + (*(ordint + n) - 1)) + drtdens_2);}
					continue;
				}
